...
const [voiceLog, setVoiceLog] = useState("Caddie System Status: Ready for voice simulation...");
const [isSunlightMode, setIsSunlightMode] = useState(false);
const [useMetres, setUseMetres] = useState(false);
...
setScores(prev => ({ ...prev, [activeHole.hole]: numericScore }));

if (numericScore > 0) {
const phrases = [
Caddie Audio Feed: "Stroke logged successfully for Hole ${activeHole.hole}.",Caddie Audio Feed: "Score updated to ${numericScore}. Stay focused.",
Caddie Audio Feed: "Confirmed. Saving card parameters."
 ];
setVoiceLog(phrases[Math.floor(Math.random() * phrases.length)]);
}
};
...
setVoiceLog(Command '${text}' not recognised. Try 'next hole' or 'score 4'.`);
}
};

return (
<GolfAppContext.Provider value={{
handicap, setHandicap,
activeHole, currentHoleIndex, setCurrentHoleIndex,
scores, updateScore,
nextHole, prevHole,
voiceLog, setVoiceLog, handleVoiceCommand,
isSunlightMode, setIsSunlightMode,
useMetres, setUseMetres,
...
const {
handicap, setHandicap,
activeHole, scores, updateScore,
nextHole, prevHole,
voiceLog, handleVoiceCommand,
isSunlightMode, setIsSunlightMode,
useMetres, setUseMetres, courseData,
...
const unitLabel = useMetres ? "M" : "YDS";
...
const relationText = totalStrokes === 0 ? "-" : relationToPar === 0 ? "E" : relationToPar > 0 ? +${relationToPar}:${relationToPar}`;
const netScore = totalStrokes > 0 ? totalStrokes - Math.round(handicap) : 0;
...
{/* SUNLIGHT ENVIRONMENT TOGGLE */}
<TouchableOpacity
style={[styles.metallicButton, { backgroundColor: isSunlightMode ? '#0F172A' : '#E2E8F0', width: '100%', marginBottom: 8 }]}
onPress={() => setIsSunlightMode(!isSunlightMode)}
>
<Text style={[styles.metallicButtonText, { color: isSunlightMode ? '#FFFFFF' : '#0F172A' }]}>
{isSunlightMode ? "🌙 SWITCH TO CLUBHOUSE DARK" : "☀️ SWITCH TO HIGH-CONTRAST SUN LIGHT"}
</Text>
</TouchableOpacity>

{/* METALLIC UNIT TOGGLE SWITCH (YARDS VS METRES) */}
<TouchableOpacity
style={[styles.metallicButton, { backgroundColor: '#F59E0B', borderColor: '#D97706', width: '100%', marginBottom: 16 }]}
onPress={() => setUseMetres(!useMetres)}
>
<Text style={[styles.metallicButtonText, { color: '#0F172A' }]}>
{useMetres ? "📐 DISPLAY UNITS: METRES (🎯 TAP FOR YARDS)" : "📐 DISPLAY UNITS: YARDS (🎯 TAP FOR METRES)"}
</Text>
</TouchableOpacity>

{/* TOP METALLIC PROFILE HEADER */}
<View style={[styles.metallicHeader, { backgroundColor: theme.cardBg, borderColor: theme.border }]}>
<Text style={[styles.headerTitle, { color: theme.textMain }]}>⛳ LUXURY CADDIEMASTER PRO</Text>
<View style={styles.handicapRow}>
<Text style={[styles.labelBright, { color: theme.textMain }]}>PLAYER HANDICAP:</Text>
<TextInput
style={[styles.metallicNumericInput, { backgroundColor: theme.inputBg, borderColor: theme.border, color: theme.textMain }]}
keyboardType="numeric"
value={String(handicap)}
onChangeText={(val) => setHandicap(parseInt(val) || 0)}
/>
</View>
</View>

{/* LIVE TOURNAMENT MATRIX STREAM */}
<View style={[styles.metallicHeader, { backgroundColor: theme.cardBg, borderColor: '#F59E0B' }]}>
