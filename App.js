import React, { useState, useEffect } from 'react';
import { StyleSheet, View, Text, Image, ScrollView, TouchableOpacity, SafeAreaView, Modal, FlatList } from 'react-native';
import AsyncStorage from '@react-native-async-storage/async-storage';

// ==========================================
// 1. GOLF COURSE DATA (CAPRICORN RESORT)
// ==========================================
const COURSE_NAME = "Capricorn Resort";

const HOLE_DATA = [
  {
    holeNumber: 1, par: 5, si: 5, distanceToCentre: 498, distanceFront: 480, distanceBack: 512,
    caddieText: "498 metres to the centre. With a slight headwind, I'd suggest driver. Aim just right of centre.",
    recommendedClub: "Driver", estCarry: 230,
    // When you have image files ready, they can go here. For now, it will safely load a dark placeholder layout.
    aerialImage: null, 
    markers: { green: { top: '10%', left: '50%' }, tee: { top: '80%', left: '50%' } }
  },
  {
    holeNumber: 2, par: 4, si: 11, distanceToCentre: 365, distanceFront: 350, distanceBack: 380,
    caddieText: "365 metres. Watch out for the fairway bunker on the left. A smooth 3-wood is perfect here.",
    recommendedClub: "3-Wood", estCarry: 210, aerialImage: null,
    markers: { green: { top: '15%', left: '45%' }, tee: { top: '78%', left: '52%' } }
  },
  {
    holeNumber: 3, par: 3, si: 17, distanceToCentre: 145, distanceFront: 135, distanceBack: 155,
    caddieText: "Short par 3. Wind is brushing off the right. Trust a standard 7-iron to find the dance floor.",
    recommendedClub: "7-Iron", estCarry: 140, aerialImage: null,
    markers: { green: { top: '20%', left: '50%' }, tee: { top: '70%', left: '50%' } }
  },
  {
    holeNumber: 4, par: 4, si: 1, distanceToCentre: 410, distanceFront: 390, distanceBack: 425,
    caddieText: "Stroke Index 1. Long par 4. Keep your tee shot right of the center line to avoid a total block out.",
    recommendedClub: "Driver", estCarry: 230, aerialImage: null,
    markers: { green: { top: '12%', left: '48%' }, tee: { top: '82%', left: '50%' } }
  },
  {
    holeNumber: 5, par: 4, si: 7, distanceToCentre: 385, distanceFront: 370, distanceBack: 398,
    caddieText: "Welcome to Hole 5. Aim straight down the narrow neck. A controlled hybrid keeps you safe.",
    recommendedClub: "Hybrid", estCarry: 195, aerialImage: null,
    markers: { green: { top: '14%', left: '52%' }, tee: { top: '80%', left: '48%' } }
  }  ,
  {
    holeNumber: 6, par: 4, si: 13, distanceToCentre: 340, distanceFront: 325, distanceBack: 355,
    caddieText: "A shorter par 4 with a slight dogleg. Avoid the fairway bunkers on the right. A smooth hybrid or 5-wood off the tee sets up a wedge into the green.",
    recommendedClub: "5-Wood", estCarry: 200, aerialImage: null,
    markers: { green: { top: '15%', left: '38%' }, tee: { top: '85%', left: '60%' } }
  },
  {
    holeNumber: 7, par: 3, si: 15, distanceToCentre: 165, distanceFront: 150, distanceBack: 175,
    caddieText: "Mid-length par 3 with a well-guarded green. Wind usually cross-breezes from the ocean here. Take an extra club to ensure you clear the front hazard.",
    recommendedClub: "6-Iron", estCarry: 160, aerialImage: null,
    markers: { green: { top: '18%', left: '50%' }, tee: { top: '75%', left: '50%' } }
  },  ,
  {
    holeNumber: 8, par: 5, si: 3, distanceToCentre: 515, distanceFront: 495, distanceBack: 530,
    caddieText: "Monster par 5 into the prevailing wind. Keep your layup shot down the right side to avoid the water hazard winding along the left fairway.",
    recommendedClub: "Driver", estCarry: 230, aerialImage: null,
    markers: { green: { top: '10%', left: '45%' }, tee: { top: '88%', left: '55%' } }
  },
  {
    holeNumber: 9, par: 4, si: 9, distanceToCentre: 390, distanceFront: 375, distanceBack: 405,
    caddieText: "Tough finishing hole for the front nine. Aim just left of the fairway tree line. The green is elevated, so hitting an extra club on your approach is smart.",
    recommendedClub: "Driver", estCarry: 225, aerialImage: null,
    markers: { green: { top: '12%', left: '50%' }, tee: { top: '80%', left: '50%' } }
  },
  {
    holeNumber: 10, par: 4, si: 10, distanceToCentre: 375, distanceFront: 360, distanceBack: 390,
    caddieText: "Starting the back nine fresh. Fairway opens up nicely here, so let the driver rip right down the center line to setup an easy short iron.",
    recommendedClub: "Driver", estCarry: 230, aerialImage: null,
    markers: { green: { top: '14%', left: '48%' }, tee: { top: '82%', left: '52%' } }
  },
  {
    holeNumber: 11, par: 4, si: 4, distanceToCentre: 415, distanceFront: 395, distanceBack: 430,
    caddieText: "Long par 4 that requires accuracy. A deep bunker guards the left landing area. Favor the right-center side off the tee to leave an open angle.",
    recommendedClub: "Driver", estCarry: 230, aerialImage: null,
    markers: { green: { top: '11%', left: '46%' }, tee: { top: '84%', left: '54%' } }
  }

    ,
  {
    holeNumber: 12, par: 3, si: 18, distanceToCentre: 135, distanceFront: 125, distanceBack: 142,
    caddieText: "The shortest par 3 on the course, but highly technical. Club down and focus entirely on target accuracy—falling short drops you straight into sand.",
    recommendedClub: "9-Iron", estCarry: 130, aerialImage: null,
    markers: { green: { top: '22%', left: '50%' }, tee: { top: '68%', left: '50%' } }
  },
  {
    holeNumber: 13, par: 5, si: 8, distanceToCentre: 490, distanceFront: 475, distanceBack: 505,
    caddieText: "Reach-in-two opportunity if you hit a big drive! Aim for the crest of the hill. If laying up, stay short of the cross-bunkers at 100 meters out.",
    recommendedClub: "Driver", estCarry: 235, aerialImage: null,
    markers: { green: { top: '10%', left: '52%' }, tee: { top: '86%', left: '48%' } }
  },
  {
    holeNumber: 14, par: 4, si: 12, distanceToCentre: 355, distanceFront: 340, distanceBack: 368,
    caddieText: "Sharp dogleg left. Big hitters can try to cut the corner over the trees, but a controlled 3-wood to the corner is the textbook professional play.",
    recommendedClub: "3-Wood", estCarry: 215, aerialImage: null,
    markers: { green: { top: '20%', left: '35%' }, tee: { top: '80%', left: '65%' } }
  },
  {
    holeNumber: 15, par: 4, si: 2, distanceToCentre: 425, distanceFront: 405, distanceBack: 440,
    caddieText: "Ranked as the hardest par 4 on the back nine. It demands a long, straight tee shot. If you cannot reach in two, play safe and try to scramble for par.",
    recommendedClub: "Driver", estCarry: 230, aerialImage: null,
    markers: { green: { top: '12%', left: '49%' }, tee: { top: '83%', left: '51%' } }
  },
  {
    holeNumber: 16, par: 3, si: 16, distanceToCentre: 155, distanceFront: 142, distanceBack: 165,
    caddieText: "Beautiful par 3 over a native vegetation dip. The green is wide but shallow. Trust your carry distance and aim directly for the center of the green structure.",
    recommendedClub: "7-Iron", estCarry: 150, aerialImage: null,
    markers: { green: { top: '16%', left: '50%' }, tee: { top: '72%', left: '50%' } }
  },
  {
    holeNumber: 17, par: 4, si: 6, distanceToCentre: 400, distanceFront: 385, distanceBack: 412,
    caddieText: "Tee shot requires threading a narrow gap. Thick bush lines both sides of the fairway. Keep your swing smooth and prioritize placement over absolute power.",
    recommendedClub: "Driver", estCarry: 220, aerialImage: null,
    markers: { green: { top: '13%', left: '51%' }, tee: { top: '79%', left: '49%' } }
  },
  {
    holeNumber: 18, par: 5, si: 14, distanceToCentre: 480, distanceFront: 460, distanceBack: 495,
    caddieText: "Fantastic par 5 finishing hole right back toward the resort clubhouse. Water protects the front right of the green. Lay up safely left to secure a closing par or birdie putt!",
    recommendedClub: "Driver", estCarry: 235, aerialImage: null,
    markers: { green: { top: '11%', left: '44%' }, tee: { top: '85%', left: '56%' } }
  }];

    
// ==========================================
// 2. MAIN APPLICATION COMPONENT
// ==========================================
export default function App() {
  const [currentHoleIndex, setCurrentHoleIndex] = useState(0);
  const [modalVisible, setModalVisible] = useState(false);
  
  // Establish baseline scorecard structures
  const [scorecard, setScorecard] = useState(
    HOLE_DATA.map(h => ({ holeNumber: h.holeNumber, strokes: 0, putts: 0, gir: false, fairway: false, penalty: 0 }))
  );

  const hole = HOLE_DATA[currentHoleIndex];
  const currentHoleScore = scorecard[currentHoleIndex];

  // Load saved round from phone storage when app boots up
  useEffect(() => {
    async function loadSavedData() {
      try {
        const savedIndex = await AsyncStorage.getItem('currentHoleIndex');
        const savedScorecard = await AsyncStorage.getItem('scorecardData');
        if (savedIndex !== null) setCurrentHoleIndex(parseInt(savedIndex, 10));
        if (savedScorecard !== null) setScorecard(JSON.parse(savedScorecard));
      } catch (e) {
        console.log("Failed to load saved game loop state data.");
      }
    }
    loadSavedData();
  }, []);

  // Save game data automatically whenever an input counter changes
  const saveState = async (newIndex, newScorecard) => {
    try {
      await AsyncStorage.setItem('currentHoleIndex', newIndex.toString());
      await AsyncStorage.setItem('scorecardData', JSON.stringify(newScorecard));
    } catch (e) {
      console.log("Failed to auto-save game state updates.");
    }
  };

  // Adjust score metric counters up or down
  const updateMetric = (metric, operation) => {
    const updatedScorecard = [...scorecard];
    let currentValue = updatedScorecard[currentHoleIndex][metric];

    if (typeof currentValue === 'boolean') {
      updatedScorecard[currentHoleIndex][metric] = !currentValue;
    } else {
      if (operation === 'add') currentValue += 1;
      if (operation === 'subtract' && currentValue > 0) currentValue -= 1;
      updatedScorecard[currentHoleIndex][metric] = currentValue;
    }

    setScorecard(updatedScorecard);
    saveState(currentHoleIndex, updatedScorecard);
  };

  const handleNextHole = () => {
    if (currentHoleIndex < HOLE_DATA.length - 1) {
      const nextIndex = currentHoleIndex + 1;
      setCurrentHoleIndex(nextIndex);
      saveState(nextIndex, scorecard);
    }
  };

  const handlePrevHole = () => {
    if (currentHoleIndex > 0) {
      const prevIndex = currentHoleIndex - 1;
      setCurrentHoleIndex(prevIndex);
      saveState(prevIndex, scorecard);
    }
  };

  const handleResetRound = async () => {
    const freshScorecard = HOLE_DATA.map(h => ({ holeNumber: h.holeNumber, strokes: 0, putts: 0, gir: false, fairway: false, penalty: 0 }));
    setCurrentHoleIndex(0);
    setScorecard(freshScorecard);
    setModalVisible(false);
    try {
      await AsyncStorage.removeItem('currentHoleIndex');
      await AsyncStorage.removeItem('scorecardData');
    } catch (e) {
      console.log("Clear failure.");
    }
  };

  // Live total calculations
  const totalStrokes = scorecard.reduce((sum, item) => sum + item.strokes, 0);
  const totalPutts = scorecard.reduce((sum, item) => sum + item.putts, 0);
  const totalPenalties = scorecard.reduce((sum, item) => sum + item.penalty, 0);

  // Traditional Golf Score Coloring rules
  const getScoreStyle = (strokes, par) => {
    if (strokes === 0) return { text: '#94A3B8', bg: 'transparent', border: 'transparent' }; // Unplayed
    if (strokes < par) return { text: '#FFF', bg: '#EF4444', border: '#EF4444' }; // Red Badge for Birdie/Under Par
    if (strokes === par) return { text: '#FFF', bg: '#475569', border: '#475569' }; // Gray Badge for Even Par
    return { text: '#FFF', bg: '#2563EB', border: '#2563EB' }; // Blue Badge for Bogey/Over Par
  };

  return (
    <SafeAreaView style={styles.container}>
      {/* HEADER SECTION */}
      <View style={styles.header}>
        <View>
          <Text style={styles.headerTitle}>TOUR PRO ELITE</Text>
          <Text style={styles.courseSubtitle}>{COURSE_NAME}</Text>
        </View>
        <Text style={styles.weatherText}>24°C | 💨 8 km/h</Text>
      </View>

      {/* PROPORTIONAL TWO-PANEL SPLIT BODY */}
      <View style={styles.mainBody}>
        
        {/* LEFT PANEL: MAP WINDOW (42% Width) */}
        <View style={styles.mapColumn}>
          {hole.aerialImage ? (
            <Image source={hole.aerialImage} style={styles.mapImage} resizeMode="cover" />
          ) : (
            <View style={styles.mapPlaceholder}>
              <Text style={styles.placeholderLabel}>[ Hole {hole.holeNumber} Map Graphic ]</Text>
              <Text style={styles.placeholderLabel}>{hole.distanceToCentre}m to Centre</Text>
            </View>
          )}
          {/* Overlay pins mapped by coordinate ratios */}
          <View style={[styles.pinGreen, hole.markers.green]}><Text style={styles.pinText}>P</Text></View>
          <View style={[styles.pinTee, hole.markers.tee]} />
        </View>

        {/* RIGHT PANEL: CADDIE DASHBOARD (58% Width) */}
        <View style={styles.caddieColumn}>
          <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
            
            {/* Distances Dashboard Grid */}
            <View style={styles.card}>
              <View style={styles.holeRow}>
                <Text style={styles.holeTitle}>Hole {hole.holeNumber}</Text>
                <Text style={styles.holeSub}>Par {hole.par} | S.I. {hole.si}</Text>
              </View>
              
              <View style={styles.heroDistanceBox}>
                <Text style={styles.heroDistanceNum}>{hole.distanceToCentre} m</Text>
                <Text style={styles.heroDistanceLabel}>To Centre</Text>
              </View>

              <View style={styles.sideDistancesGrid}>
                <View><Text style={styles.sideLabel}>Front</Text><Text style={styles.sideNum}>{hole.distanceFront} m</Text></View>
                <View><Text style={styles.sideLabel}>Back</Text><Text style={styles.sideNum}>{hole.distanceBack} m</Text></View>
              </View>
            </View>

            {/* AI Voice Caddie Insight */}
            <View style={styles.card}>
              <Text style={styles.cardLabel}>Voice Caddie Advice</Text>
              <Text style={styles.caddieAdviceText}>"{hole.caddieText}"</Text>
            </View>

            {/* Club Choice Box */}
            <View style={styles.card}>
              <Text style={styles.cardLabel}>Recommended Club</Text>
              <Text style={styles.clubNameText}>{hole.recommendedClub}</Text>
              <Text style={styles.carryEstimatorText}>Est. Carry: {hole.estCarry}m</Text>
            </View>

          </ScrollView>
        </View>
      </View>

      {/* CORE ROUND FOOTER SCORECOUNTER SHEETS */}
      <View style={styles.scoreCounterPanel}>
        <View style={styles.counterRow}>
          
          {/* Main Strokes Input */}
          <View style={styles.counterBlock}>
            <Text style={styles.counterLabel}>Strokes</Text>
            <View style={styles.controlRow}>
              <TouchableOpacity onPress={() => updateMetric('strokes', 'subtract')} style={styles.minusBtn}><Text style={styles.btnSymbol}>-</Text></TouchableOpacity>
              <Text style={styles.counterNum}>{currentHoleScore.strokes}</Text>
