...
const HOLE_DATA = [
  ...
  },
  {
    ...
    elevationChange: -2,
    safeMissZone: "Lay up far left. Water completely locks down the front right.",    caddieText: "Fantastic par 5 finishing hole right back toward the resort clubhouse. Water protects the front right of the green. Lay up safely left to secure a closing par or birdie putt!",
    recommendedClub: "Driver", estCarry: 235, aerialImageUrl: FALLBACK_MAP,
...
export default function App() {
  ...
  const airTemperatureC = 24; 

  const [scorecard, setScorecard] = useState(
    HOLE_DATA.map(h => ({ holeNumber: h.holeNumber, strokes: 0, putts: 0, gir: false, fairway: false, penalty: 0 }))
  );

  const hole = HOLE_DATA[currentHoleIndex];
  const currentHoleScore = scorecard[currentHoleIndex];

  // Professional Plays-Like Mathematical Variance Engine
  const calculatePlaysLikeDistance = () => {
    let base = hole.distanceToCentre;
    base += hole.elevationChange;
    if (windDirection === "headwind") base += (windSpeedKmH * 0.8); 
    if (windDirection === "tailwind") base -= (windSpeedKmH * 0.5); 
    if (airTemperatureC < 15) base += 4; 
    if (airTemperatureC > 30) base -= 3; 
    return Math.round(base);
  };

  const playsLikeDistance = calculatePlaysLikeDistance();

  // Storage Persistence Operations
  useEffect(() => {
    async function loadSavedData() {
      try {
        const savedIndex = await AsyncStorage.getItem('currentHoleIndex');
        const savedScorecard = await AsyncStorage.getItem('scorecardData');
        if (savedIndex !== null) setCurrentHoleIndex(parseInt(savedIndex, 10));
        if (savedScorecard !== null) setScorecard(JSON.parse(savedScorecard));
      } catch (e) {
  ...
  const saveState = async (newIndex, newScorecard) => {
    try {
      await AsyncStorage.setItem('currentHoleIndex', newIndex.toString());
      await AsyncStorage.setItem('scorecardData', JSON.stringify(newScorecard));
    } catch (e) {
      console.log("Failed to save state.");
    }
  };

  const updateMetric = (metric, operation) => {
    const updatedScorecard = [...scorecard];
    let currentValue = updatedScorecard[currentHoleIndex][metric];
    if (operation === 'add') currentValue += 1;
    if (operation === 'subtract' && currentValue > 0) currentValue -= 1;
    updatedScorecard[currentHoleIndex][metric] = currentValue;
    setScorecard(updatedScorecard);
    saveState(currentHoleIndex, updatedScorecard);
  };
  ...
  const handlePrevHole = () => {
    if (currentHoleIndex > 0) {
      ...
      saveState(prevIndex, scorecard);
    }
  };

  const handleResetRound = async () => {
    const freshScorecard = HOLE_DATA.map(h => ({ holeNumber: h.holeNumber, strokes: 0, putts: 0, gir: false, fairway: false, penalty: 0 }));
    setCurrentHoleIndex(0);
    setScorecard(freshScorecard);
    setModalVisible(false);
    try {
      await AsyncStorage.removeItem('currentHoleIndex');
      await AsyncStorage.removeItem('scorecardData');
    } catch (e) {
      console.log("Clear failure.");
    }
  };

  const totalStrokes = scorecard.reduce((sum, item) => sum + item.strokes, 0);
  const totalPutts = scorecard.reduce((sum, item) => sum + item.putts, 0);
  const totalPenalties = scorecard.reduce((sum, item) => sum + item.penalty, 0);

  const getScoreStyle = (strokes, par) => {
    if (strokes === 0) return { text: '#94A3B8', bg: 'transparent' };
    if (strokes < par) return { text: '#FFF', bg: '#EF4444' }; 
    if (strokes === par) return { text: '#FFF', bg: '#475569' }; 
    return { text: '#FFF', bg: '#2563EB' }; 
  };
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.header}>
        <View>
          <Text style={styles.headerTitle}>TOUR PRO ELITE</Text>
          <Text style={styles.courseSubtitle}>{COURSE_NAME}</Text>
        </View>
        <Text style={styles.weatherText}>24°C | 💨 15km/h {windDirection.toUpperCase()}</Text>
      </View>

      {/* DUAL CANVAS INTERACTIVE WRAPPER */}
      <View style={styles.mainBody}>
        {/* LEFT MAP WINDOW FRAME (42% SIZING) */}
        <View style={styles.mapColumn}>
          <Image source={{ uri: hole.aerialImageUrl }} style={styles.mapImage} resizeMode="cover" />
          <View style={[styles.pinGreen, hole.markers.green]}><Text style={styles.pinText}>P</Text></View>
          <View style={[styles.pinTee, hole.markers.tee]} />
        </View>

        {/* RIGHT METRICS LOG ENGINE (58% SIZING) */}
        <View style={styles.caddieColumn}>
          <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
            {/* PRO VARIANCE CALCULATION DATA SHIELD */}
            <View style={[styles.card, { borderColor: '#F59E0B', borderWidth: 1.5 }]}>
              <View style={styles.holeRow}>
                <Text style={[styles.holeTitle, { color: '#F59E0B' }]}>Hole {hole.holeNumber}</Text>
                <Text style={styles.holeSub}>Par {hole.par} | S.I. {hole.si}</Text>
              </View>
              <Text style={styles.playsLikeNum}>{playsLikeDistance} m</Text>
              <Text style={styles.playsLikeLabel}>⚡️ PRO PLAYS-LIKE TARGET</Text>
              <View style={styles.varianceDataGrid}>
                <Text style={styles.varianceText}>GPS: {hole.distanceToCentre}m</Text>
                <Text style={styles.varianceText}>Slope: {hole.elevationChange > 0 ? `+${hole.elevationChange}` : hole.elevationChange}m</Text>
                <Text style={styles.varianceText}>Wind: +12m</Text>
              </View>
            </View>

            {/* STRATEGIC MISS BOUNDARY ASSISTANCE PROFILE */}
            <View style={styles.card}>
              <Text style={[styles.cardLabel, { color: '#10B981' }]}>🛡️ STRATEGIC MISS DIRECTORY</Text>
              <Text style={styles.missZoneText}>{hole.safeMissZone}</Text>
            </View>

            {/* REALISTIC CADDIE LOG DIALOGUE */}
            <View style={styles.card}>
              <Text style={styles.cardLabel}>Voice Caddie Strategy</Text>
              <Text style={styles.caddieAdviceText}>"{hole.caddieText}"</Text>
            </View>

            {/* FLIGHT DELIVERY CLUB SELECTION WRAPPER */}
            <View style={styles.card}>
              <Text style={styles.cardLabel}>Recommended Club Delivery</Text>
              <Text style={styles.clubNameText}>{hole.recommendedClub}</Text>
              <Text style={styles.carryEstimatorText}>Required Carry: {playsLikeDistance - 15}m (Est. Total: {playsLikeDistance}m)</Text>
            </View>
          </ScrollView>
        </View>
      </View>

      {/* LIVE SCORING INCREMENT COUNTER SHEETS */}
      <View style={styles.scoreCounterPanel}>
        <View style={styles.counterRow}>
          <View style={styles.counterBlock}>
            <Text style={styles.counterLabel}>Strokes</Text>
            <View style={styles.controlRow}>
              <TouchableOpacity onPress={() => updateMetric('strokes', 'subtract')} style={styles.minusBtn}><Text style={styles.btnSymbol}>-</Text></TouchableOpacity>
              <Text style={styles.counterNum}>{currentHoleScore.strokes}</Text>
              <TouchableOpacity onPress={() => updateMetric('strokes', 'add')} style={styles.plusBtn}><Text style={styles.btnSymbol}>+</Text></TouchableOpacity>
            </View>
          </View>
          <View style={styles.counterBlock}>
            <Text style={styles.counterLabel}>Putts</Text>
            <View style={styles.controlRow}>
              <TouchableOpacity onPress={() => updateMetric('putts', 'subtract')} style={styles.minusBtn}><Text style={styles.btnSymbol}>-</Text></TouchableOpacity>
              <Text style={styles.counterNum}>{currentHoleScore.putts}</Text>
              <TouchableOpacity onPress={() => updateMetric('putts', 'add')} style={styles.plusBtn}><Text style={styles.btnSymbol}>+</Text></TouchableOpacity>
            </View>
          </View>
          <View style={styles.counterBlock}>
            <Text style={styles.counterLabel}>Penalty</Text>
            <View style={styles.controlRow}>
              <TouchableOpacity onPress={() => updateMetric('penalty', 'subtract')} style={styles.minusBtn}><Text style={styles.btnSymbol}>-</Text></TouchableOpacity>
              <Text style={styles.counterNum}>{currentHoleScore.penalty}</Text>
              <TouchableOpacity onPress={() => updateMetric('penalty', 'add')} style={styles.plusBtn}><Text style={styles.btnSymbol}>+</Text></TouchableOpacity>
            </View>
          </View>
        </View>
        <View style={styles.actionRow}>
          <TouchableOpacity onPress={handlePrevHole} disabled={currentHoleIndex === 0} style={[styles.navBtn, currentHoleIndex === 0 && { opacity: 0.4 }]}>
            <Text style={styles.navBtnText}>⏮ Hole</Text>
          </TouchableOpacity>
          <TouchableOpacity onPress={() => setModalVisible(true)} style={styles.viewScorecardBtn}>
            <Text style={styles.viewScorecardText}>📋 View Scorecard</Text>
          </TouchableOpacity>
          <TouchableOpacity onPress={handleNextHole} disabled={currentHoleIndex === HOLE_DATA.length - 1} style={[styles.navBtn, currentHoleIndex === HOLE_DATA.length - 1 && { opacity: 0.4 }]}>
            <Text style={styles.navBtnText}>Next Hole ⏭</Text>
