...
const WARMUP_STEPS = [
  { title: 'LOOSEN UP', text: '2–3 minutes. Easy shoulder turns, hip rotations and gentle swings. Start at about 50% effort and let the body loosen naturally.' },
  { title: 'SHORT WEDGES', text: 'Hit 6–8 short wedges. Begin with half swings, find centred contact and a consistent landing point, then build the length gradually.' },
  { title: 'MID IRONS', text: 'Hit 5–6 mid irons at normal rhythm. Check strike, start line and balance. Do not chase technical changes before the round.' },
  { title: 'LONG CLUB', text: 'Hit 4–5 long irons, hybrids or fairway woods. Build to normal speed, confirm your stock flight and finish in balance.' },
  { title: 'DRIVER', text: 'Hit 4–6 committed drives. Start at controlled speed, then reach normal playing speed. Finish by rehearsing the first-tee shot you intend to play.' },
  { title: 'CHIPPING', text: 'Hit 6–10 chips from at least two lies. Pick landing spots, vary trajectory and confirm how the ball is reacting on the day.' },
  { title: 'PUTTING', text: 'Start with short putts for start line, then 4–6 long putts for pace. Finish with a few putts you expect to hole.' },
...
Alert.alert(
'Course card saved',
`${cleanCourseName} · ${teeColour.toUpperCase()} tees`
);
}
...
Alert.alert(
'Round finished',
`${courseName} · ${totalScore} shots · ${toParText()}`,
[
{
text: 'HOME',
onPress: () =>
...
Alert.alert(
'Shot saved',
`${shot.club} · ${shot.distance} ${primaryLabel()} · ${shot.result}`
);
};
...
Alert.alert(
'Practice saved',
`${practiceType} · ${practiceClub} · ${practiceSuccess}% success`
);
};
...
title
  ...
  <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
    ...
    <Text style={styles.courseSwipeHint}>SWIPE ↔</Text>
  </View>
  ...
  return (
    <View style={styles.groupNineCard}>
      ...
      <View style={[styles.groupScoreRow, styles.groupTotalRow]}>
        ...
        <Text style={styles.groupSiCell}>—</Text>
...
const renderGroupScorecard = () => (
  <View style={styles.groupScorePage}>
    <View style={styles.groupScoreTitleRow}>
      ...
      >
        <Text style={styles.groupBackText}>‹</Text>
      </TouchableOpacity>
    ...
    <Text style={styles.groupManualNote}>
      MANUAL SCORE ENTRY · TPE OWNER SCORES THE FOUR-PLAYER GROUP
    </Text>
    ...
    <Text style={styles.groupScoreFootnote}>
      ...
      SCORES ARE MANUAL — VOICE CADDIE DOES NOT ENTER GROUP SCORES.
    </Text>
      ...
      <View style={styles.courseCardHeader}>
        <View style={{ flex: 1 }}>
          ...
          <Text style={styles.courseCardSubtitle}>
            TAP PAR TO CHANGE · ENTER DISTANCE + SI
          </Text>
        </View>
      ...
      <View style={styles.courseCardFooter}>
        <View>
          ...
          <Text style={styles.courseFooterValue}>
            {courseDistanceTotal || '—'} {primaryLabel()}
          </Text>
        </View>
        ...
        <View style={styles.courseFooterRight}>
          ...
          <Text style={styles.courseFooterValue}>
            {teeColour ? teeColour.toUpperCase() : '—'}
          </Text>
        </View>
      </View>
    </View>
...
<VoicePanel mode="ROUND" />
    ...
    <View style={styles.callCard}>
      ...
      <Text style={styles.callText}>
        {infoMode === 'PRO'
          ? `${formatDistance(playsLikeYards)} · ${getWindLabel()} · ${getLieLabel()}`
          : `${primaryDistance(playsLikeYards)} ${primaryLabel()} · ${actualClub.name}`}
      </Text>
    </View>
...
const renderScore = () => (
  <>
    <View style={styles.hole46TopRow}>
      <View>
        <Text style={styles.hole46Eyebrow}>LIVE ROUND</Text>
        <Text style={styles.hole46Title}>HOLE {currentHole}</Text>
        <Text style={styles.hole46Course}>{courseName || 'COURSE'}</Text>
      </View>
      <View style={styles.hole46ParBox}>
        <Text style={styles.hole46ParLabel}>PAR</Text>
        <Text style={styles.hole46ParValue}>{currentCourseHole.par || '-'}</Text>
      </View>
    </View>

    {!activeRound ? (
      <View style={styles.noRoundCard}>
        <Text style={styles.noRoundText}>No live round started.</Text>
        <TouchableOpacity style={styles.secondaryButton} onPress={() => changePage('COURSE')}>
          <Text style={styles.secondaryButtonText}>START A ROUND</Text>
        </TouchableOpacity>
      </View>
    ) : (
      <>
        <View style={styles.hole46DistanceCard}>
          <View style={styles.hole46DistanceLeft}>
            <Text style={styles.hole46Small}>HOLE DISTANCE</Text>
            <Text style={styles.hole46Distance}>
              {currentCourseHole.distance || '-'}
              <Text style={styles.hole46Unit}> {primaryLabel()}</Text>
            </Text>
          </View>
          <View style={styles.hole46DistanceRight}>
            <Text style={styles.hole46Small}>PLAYS LIKE</Text>
            <Text style={styles.hole46Plays}>{primaryDistance(playsLikeYards)} {primaryLabel()}</Text>
            <Text style={styles.hole46Club}>{actualClub.name}</Text>
          </View>
        </View>

        <View style={styles.hole46ActionRow}>
          <TouchableOpacity style={styles.hole46PrimaryAction} onPress={() => changePage('CADDIE')}>
            <Text style={styles.hole46PrimaryActionText}>ASK CADDIE</Text>
          </TouchableOpacity>
          <TouchableOpacity style={styles.hole46Action} onPress={() => changePage('TRACK')}>
            <Text style={styles.hole46ActionText}>TRACK SHOT</Text>
          </TouchableOpacity>
        </View>

        <View style={styles.hole46CaddieCard}>
          <Text style={styles.hole46SectionLabel}>VOICE CADDIE</Text>
          <VoicePanel mode="ROUND" />
          <View style={styles.hole46AdviceBox}>
            <Text style={styles.hole46AdviceClub}>{actualClub.name}</Text>
            <Text style={styles.hole46AdviceText}>
              {infoMode === 'PRO'
                ? `${formatDistance(playsLikeYards)} · ${getWindLabel()} · ${getLieLabel()}`
                : `${primaryDistance(playsLikeYards)} ${primaryLabel()} · ${actualClub.name}`}
            </Text>
        ...
        <View style={styles.hole46ScoreCard}>
          <View style={styles.hole46ScoreHeader}>
            <Text style={styles.hole46SectionLabel}>SCORE THIS HOLE</Text>
            <TouchableOpacity onPress={() => changePage('SCORECARD')}>
              <Text style={styles.hole46ScorecardLink}>VIEW SCORECARD</Text>
            </TouchableOpacity>
          </View>
          ...
          <View style={styles.twoColumn}>
            <View style={styles.column}>
              ...
              <TextInput
                ...
                onChangeText={(text) => updateHole('score', text.replace(/[^0-9]/g, ''))}
                keyboardType="number-pad"
                maxLength={2}
              />
            </View>
            <View style={styles.column}>
              ...
              <TextInput
                ...
                onChangeText={(text) => updateHole('putts', text.replace(/[^0-9]/g, ''))}
                keyboardType="number-pad"
                maxLength={1}
              />
            </View>
          ...
          <Text style={styles.fieldLabel}>G.I.R.</Text>
          <Selector options={['YES', 'NO']} value={currentHoleData.gir} onChange={(value) => updateHole('gir', value)} />

          <Text style={styles.fieldLabel}>FAIRWAY</Text>
          <Selector options={['HIT', 'LEFT', 'RIGHT', 'N/A']} value={currentHoleData.fairway} onChange={(value) => updateHole('fairway', value)} />
        </View>

        <View style={styles.hole46RoundSummary}>
          <View style={styles.hole46SummaryItem}><Text style={styles.hole46Small}>TOTAL</Text><Text style={styles.hole46SummaryValue}>{totalScore || '-'}</Text></View>
          <View style={styles.hole46SummaryItem}><Text style={styles.hole46Small}>TO PAR</Text><Text style={styles.hole46SummaryValue}>{toParText()}</Text></View>
          <View style={styles.hole46SummaryItem}><Text style={styles.hole46Small}>PUTTS</Text><Text style={styles.hole46SummaryValue}>{totalPutts}</Text></View>
        </View>

        <View style={styles.hole46NavRow}>
          <TouchableOpacity
            style={[styles.hole46NavButton, currentHole === 1 && styles.hole46NavDisabled]}
            disabled={currentHole === 1}
            onPress={() => setLiveHole(Math.max(1, currentHole - 1))}
          >
            <Text style={styles.hole46NavText}>‹ PREVIOUS</Text>
          </TouchableOpacity>
          <TouchableOpacity
            style={[styles.hole46NavButton, currentHole === 18 && styles.hole46NavDisabled]}
            disabled={currentHole === 18}
            onPress={() => setLiveHole(Math.min(18, currentHole + 1))}
          >
            <Text style={styles.hole46NavText}>NEXT ›</Text>
          </TouchableOpacity>
        </View>
      </>
    )}
...
<Text style={styles.smallGrey}>
{shot.date} · {shot.distance} {primaryLabel()} · {getShotLieLabel(shot.lie)} · {shot.result}
</Text>
</View>
</View>
))}
...
{practiceClub}
{practiceTargetDistance
? ` · ${practiceTargetDistance} ${primaryLabel()}`
: ''}
{practiceShotShape
? ` · ${practiceShotShape}`
: ''}
...
{item.date} · {item.shotsRecorded} shots ·{' '}
{item.goodShots} good · {item.misses} misses
</Text>
...
<View style={styles.routineHabitCard}>
  <Text style={styles.routineHabitTop}>PRACTICE LIKE YOU’RE PLAYING</Text>
...
const styles = StyleSheet.create({
  hole46TopRow: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 },
  hole46Eyebrow: { color: '#D8B64D', fontSize: 9, fontWeight: '900', letterSpacing: 1.4 },
  hole46Title: { color: '#FFFFFF', fontSize: 32, fontWeight: '900', marginTop: 2 },
  hole46Course: { color: '#8D9A92', fontSize: 11, fontWeight: '700', marginTop: 2 },
  hole46ParBox: { minWidth: 66, borderWidth: 1, borderColor: '#D8B64D', borderRadius: 12, paddingVertical: 8, alignItems: 'center' },
  hole46ParLabel: { color: '#8D9A92', fontSize: 8, fontWeight: '900' },
  hole46ParValue: { color: '#D8B64D', fontSize: 25, fontWeight: '900' },
  hole46DistanceCard: { backgroundColor: '#102218', borderWidth: 1, borderColor: '#1D3A2A', borderRadius: 14, padding: 16, flexDirection: 'row', marginBottom: 10 },
  hole46DistanceLeft: { flex: 1 },
  hole46DistanceRight: { flex: 1, alignItems: 'flex-end', borderLeftWidth: 1, borderLeftColor: '#1D3A2A', paddingLeft: 14 },
  hole46Small: { color: '#7E8A84', fontSize: 8, fontWeight: '900', letterSpacing: 0.8 },
  hole46Distance: { color: '#FFFFFF', fontSize: 30, fontWeight: '900', marginTop: 5 },
  hole46Unit: { color: '#8D9A92', fontSize: 11, fontWeight: '800' },
  hole46Plays: { color: '#D8B64D', fontSize: 20, fontWeight: '900', marginTop: 5 },
  hole46Club: { color: '#FFFFFF', fontSize: 13, fontWeight: '900', marginTop: 3 },
  hole46ActionRow: { flexDirection: 'row', gap: 8, marginBottom: 10 },
  hole46PrimaryAction: { flex: 1, minHeight: 48, borderRadius: 10, backgroundColor: '#D8B64D', alignItems: 'center', justifyContent: 'center' },
  hole46PrimaryActionText: { color: '#07130D', fontSize: 11, fontWeight: '900' },
  hole46Action: { flex: 1, minHeight: 48, borderRadius: 10, borderWidth: 1, borderColor: '#35513F', backgroundColor: '#102218', alignItems: 'center', justifyContent: 'center' },
  hole46ActionText: { color: '#FFFFFF', fontSize: 11, fontWeight: '900' },
  hole46CaddieCard: { backgroundColor: '#102218', borderWidth: 1, borderColor: '#1D3A2A', borderRadius: 14, padding: 14, marginBottom: 10 },
  hole46SectionLabel: { color: '#D8B64D', fontSize: 10, fontWeight: '900', letterSpacing: 1 },
  hole46AdviceBox: { backgroundColor: '#0B1A12', borderRadius: 10, borderWidth: 1, borderColor: '#294133', padding: 12, marginTop: 10 },
  hole46AdviceClub: { color: '#FFFFFF', fontSize: 18, fontWeight: '900' },
  hole46AdviceText: { color: '#A6B0AA', fontSize: 11, lineHeight: 17, marginTop: 4 },
  hole46ScoreCard: { backgroundColor: '#102218', borderWidth: 1, borderColor: '#1D3A2A', borderRadius: 14, padding: 14, marginBottom: 10 },
  hole46ScoreHeader: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' },
  hole46ScorecardLink: { color: '#FFFFFF', fontSize: 9, fontWeight: '900', textDecorationLine: 'underline' },
  hole46RoundSummary: { flexDirection: 'row', backgroundColor: '#0B1A12', borderWidth: 1, borderColor: '#1D3A2A', borderRadius: 12, marginBottom: 10 },
  hole46SummaryItem: { flex: 1, alignItems: 'center', paddingVertical: 12 },
  hole46SummaryValue: { color: '#FFFFFF', fontSize: 18, fontWeight: '900', marginTop: 3 },
  hole46NavRow: { flexDirection: 'row', gap: 8, marginTop: 2 },
  hole46NavButton: { flex: 1, minHeight: 48, borderWidth: 1, borderColor: '#D8B64D', borderRadius: 10, alignItems: 'center', justifyContent: 'center' },
  hole46NavDisabled: { opacity: 0.3 },
  hole46NavText: { color: '#D8B64D', fontSize: 10, fontWeight: '900' },
