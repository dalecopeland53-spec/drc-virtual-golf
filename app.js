...
if(
txt.includes('headwind')||
txt.includes('into the wind')||
wind==='HEAD'
){
...
if(
txt.includes('tailwind')||
txt.includes('wind behind')||
wind==='TAIL'
){
...
setAnswer(
`${caddieName}: ${d} metres, ${selectedLie.toLowerCase()}. Plays about ${adjusted} metres. ${club}.${note}`
);
};
...
setAnswer(
`ERROR: ${err}${code!==undefined?' | CODE: '+code:''}${message?' | '+message:''}`
);
});
...
setAnswer('Starting speech recognition...');
...
onPress={()=>
setDistance(
String(Math.max(1,(parseInt(distance)||0)-1))
)
}>
...
style={s.smallButton}
onPress={()=>
setDistance(String((parseInt(distance)||0)+1))
}>
<Text style={s.smallButtonText}>+</Text>
</TouchableOpacity>
</View>
...
onPress={()=>makeAdvice('')}
>
<Text style={s.askButtonText}>
ASK {caddieName.toUpperCase()}
</Text>
</TouchableOpacity>
</View>

<Text style={s.footer}>
DRC Virtual Golf • Standalone Native Build
</Text>
</ScrollView>
</SafeAreaView>
);
